...
class OrderProcessorRepositoryImpl(OrderProcessorRepository):
    ...
    def create_product(self, product):
        ...
        # Check if the product is already inactive
            cursor.execute("SELECT * FROM products WHERE product_id = ? AND is_active = 1", (product['product_id'],))
            if cursor.fetchone() is None:
                print(f"Product ID {product['product_id']} is inactive. Reactivate it before adding.")
                return False

            cursor.execute("INSERT INTO products (product_id, name, price, description, stockQuantity) VALUES (?, ?, ?, ?, ?)",
                       (product['product_id'], product['name'], product['price'], product['description'], product['stockQuantity']))
            self.connection.commit()
            return True
        except Exception as e:
    ...
    def delete_customer(self, customer_id: int) -> bool:
        ...
        # Delete all items in the customer's cart
